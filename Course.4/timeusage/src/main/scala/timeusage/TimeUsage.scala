...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = StructType(
    StructField(columnNames.head, StringType, nullable = false)
      :: columnNames.tail.map(column => StructField(column, DoubleType, nullable = false))
  )
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val desiredGroups = List(
      List("t01", "t03", "t11", "t1801", "t1803", "tuc"),
      List("t05", "t1805"),
      List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    ).zipWithIndex

    def columnHelper(groupMapping: List[(List[String], Int)], column: String): Option[Int] = {
      groupMapping.find(group => group._1.exists(prefix => column.startsWith(prefix))) match {
        case Some(value) => Option(value._2)
        case _ => None
      }
    }

    val groups = columnNames.groupBy(column => columnHelper(desiredGroups, column)).filter(_._1.isDefined)
      .map(pair => (pair._1.get, pair._2.map(colName => new Column(colName))))
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    s"SELECT working, sex, age, ROUND(AVG(primaryNeeds),1) as primaryNeeds, ROUND(AVG(work),1) as work,  ROUND(AVG(other),1) as other " +
      s"FROM $viewName " +
      s"GROUP BY working, sex, age ORDER BY working, sex, age"
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    summed.groupByKey(r => (r.working, r.sex, r.age))
      ...
      .map(c => TimeUsageRow(c._1._1, c._1._2, c._1._3, c._2, c._3, c._4))
