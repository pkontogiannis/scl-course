...
object Huffman {
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs match {
    case List() => List()
    case h :: t => Leaf(h._1, h._2) :: makeOrderedLeafList(t) sortWith (_.weight < _.weight)
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case first :: second :: c => makeCodeTree(first, second) :: c sortWith (weight(_) < weight(_))
    case _ => trees
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def helper(tr: CodeTree, b: List[Bit]): List[Char] = tr match {
      case Leaf(ch, _) => if (b.isEmpty) List(ch) else ch :: helper(tree, b)
      case Fork(l, r, _, _) => if (b.head == 0) helper(l, b.tail) else helper(r, b.tail)
    }
