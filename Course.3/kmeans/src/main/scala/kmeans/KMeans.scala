...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val ptsMap = points.par.groupBy(findClosest(_, means))
    means.par.map(mn => mn -> ptsMap.getOrElse(mn, GenSeq())).toMap
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.par.map(om => findAverage(om, classified(om)))
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    (oldMeans zip newMeans).forall {
      case (p1, p2) => p1.squareDistance(p2) <= eta
    }
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    val nMeans = update(classified, means)
    if (!converged(eta)(means, nMeans))
      kMeans(points, nMeans, eta)
    else
      nMeans
  }
}
